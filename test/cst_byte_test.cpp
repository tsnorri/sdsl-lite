...
typedef Types<
         ...
         cst_sada<csa_rao<csa_rao_spec<2, 0>>, lcp_bitcompressed<>>,
         cst_sada<csa_rao<csa_rao_spec<4, 0>>, lcp_bitcompressed<>>,
         cst_sada<csa_rao<csa_rao_spec<5, 0>>, lcp_bitcompressed<>>,
...
TYPED_TEST(cst_byte_test, edge)
{
    ...
    if (cst.csa.size() > 0) {
        ...
        for (size_type i=0; i<max_depth; ++i) {
            ASSERT_EQ(data[i], cst.edge(v, i+1))<<" i="<<i;//<<" v="<<v; FIXME enable
        }
        v = cst.parent(v);
